...
grid.newpage()
grob <- linesGrob(x = c(.4, .8, .8, .2, .6), y = c(.3, .3, .8, .8, .6))
polylineoffset(grob, 0.1, jointype="square", endtype = "opensquare")
grid.draw(grob)

grid.newpage()
grob <- linesGrob(x = c(.4, .8, .8, .2, .6), y = c(.3, .3, .8, .8, .6))
polylineoffset(grobCoords(grob, F), 0.1, jointype="square", endtype = "opensquare")
grid.draw(grob)

#Test generic polyoffset
grid.newpage()
grob <- rectGrob(width = 0.5, height = 0.5)
polyoffset(grob, 0.1, jointype="square", endtype = "opensquare")
grid.draw(grob)

grid.newpage()
grob <- rectGrob(width = 0.5, height = 0.5)
polyoffset(grobCoords(grob, T), unit(0.1, "inch"), jointype="square", endtype = "opensquare")
grid.draw(grob)
